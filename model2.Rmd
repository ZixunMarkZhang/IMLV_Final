...
The following PDP plot shows the relationship between scaled interest rate index and predicted probability of y=yes. The curve shows overall decreasing trend, which is consistent with PDP plot in logistic regression, with a increase at scaled interest rate index equal to negative 1. If we combine this plot with the EDA overlapped histogram in EDA, the pattern is reasonable. The EDA shows deceasing and then increasing in probability in scaled interest rate index \<-1, which is the same as what the PDP shows. When the index is larger than 0, the probability is even lower, hence we see another decreasing in the PDP plot.
...
The PDP plot following is the PDP plot of whether the person was contacted via telephone or cellular against the target variable. We can observe that if the contact is by telephone, holding all other variable constant, the person is slightly more likely to have term deposit at the bank. However, the shapley value plots indicates that the lower quantile, median, upper quantile contribution of this variable toward target variable is 0. In other words, decision tree does not create a meaningful boundary distinguishing these two variables. That is also why the PDP plot here shows a small difference in predicted probability.
...
The following PDP plot shows the relationship between scaled number of contact in this campaign and predicted probability of y=yes. The curve shows overall increasing trend,with a decreasing and then increasing at campaign=2.5. If we combine this plot with the EDA overlapped histogram in EDA, the pattern is reasonable. The EDA shows shows a overall increasing in proportion of y=yes with the increase in scaled number of contact, however, the number of yes case is almost zero when scaled campaign reaches 2.5, which explains why we observe such decrease in PDP plot.
...
The following PDP plot shows the relationship between scaled age and predicted probability of y=yes. The curve shows overall decreasing with a major drop in predicted probability at scaled age = 2. If we combine this plot with the EDA overlapped histogram in EDA, the pattern two pattern shows conflict as EDA indicates increase in predicted probability when scaled age is larger than 2. We may explain this conflict by the lack of data of scaled age larger than 2, which is a common problem for PDP plot.
...
The following cell shows the test accuracy of decision tree model. The model accuracy on test data is 0.8896359, which indicates it is a good model. But the AUC roc value is lower than other two values. The curve is also less bent toward the upper left corner comparing with the roc curves of logistic regression. We may conclude that the model only performs well in terms of overall accuracy.

```{r,cache=TRUE,message=FALSE}
DTpred = predict(DTmod,test_dat)[,2]
mean((DTpred>0.5)==(test_dat$y=='yes'))
auc(roc(test_dat$y,DTpred))
```
...
At the end, we may conclude that interest rate index, price index, way of contact, number of contact in previous campaign, age, and the outcome of previous campaign are the most dominant variables determine the prediction of whether the person has term deposit with decision tree model.
