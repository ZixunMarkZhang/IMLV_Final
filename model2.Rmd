# [Decision Tree]
```{r}
library(rpart)
DTmod <- rpart(y ~ ., data = train_dat,cp=0,method='class')
pred <- function(model, newdata) {
  predict(model, newdata = newdata, type = "prob")[, 2]
}
plotcp(DTmod)
```


```{r}
DTmod <- rpart(y ~ ., data = train_dat,cp=0.002,method='class')

DTpred = predict(DTmod,test_dat)[,2]
```

```{r}
library(rpart.plot)
rpart.plot(DTmod)
```

```{r}

shap_values <- fastshap::explain(
  DTmod, 
  X = train_dat,           
  feature_names = colnames(train_dat |> dplyr::select(-y)),
  pred_wrapper = pred, 
  nsim = 5,
  newdata = test_dat
)

library(ggplot2)
library(reshape2)
ggplot(melt(shap_values),aes(value, reorder(variable, value, FUN = median)))+
  geom_boxplot()
```
```{r}
library(pdp)
pdp_hit_rf = partial(DTmod,pred.var = 'euribor3m')
g <- ggplot(pdp_hit_rf, aes(euribor3m, yhat)) +
  geom_line() +
  geom_rug(data = train_dat, aes(euribor3m), inherit.aes = FALSE, alpha = .5,
  color = "red") +
  theme_bw(16)
g+ylab('prob')
```
```{r}
library(pdp)
pdp_hit_rf = partial(DTmod,pred.var = 'cons.conf.idx')
g <- ggplot(pdp_hit_rf, aes(cons.conf.idx, yhat)) +
  geom_line() +
  geom_rug(data = train_dat, aes(cons.conf.idx), inherit.aes = FALSE, alpha = .5,
  color = "red") +
  theme_bw(16)
g+ylab('prob')
```

```{r}
library(pdp)
pdp_hit_rf = partial(DTmod,pred.var = 'cons.price.idx')
g <- ggplot(pdp_hit_rf, aes(cons.price.idx, yhat)) +
  geom_line() +
  geom_rug(data = train_dat, aes(cons.price.idx), inherit.aes = FALSE, alpha = .5,
  color = "red") +
  theme_bw(16)
g+ylab('prob')
```

```{r}
library(pdp)
pdp_hit_rf = partial(DTmod,pred.var = 'euribor3m')
g <- ggplot(pdp_hit_rf, aes(euribor3m, yhat)) +
  geom_line() +
  geom_rug(data = train_dat, aes(euribor3m), inherit.aes = FALSE, alpha = .5,
  color = "red") +
  theme_bw(16)
g+ylab('prob')
```

```{r}
get_rates <- function(threshold, actual, response) {
  predicted <- ifelse(response < threshold, 0, 1)
  
  TP <- sum(actual == 1 & predicted == 1)
  FP <- sum(actual == 0 & predicted == 1)
  TN <- sum(actual == 0 & predicted == 0)
  FN <- sum(actual == 1 & predicted == 0)
  
  tpr <- TP/(TP + FN)
  fpr <- FP/(TN + FP)  
  
  data.frame(threshold, tpr, fpr)
  
}


rocdf <- map_df(seq(0, 1, .01), get_rates, test_dat$y==TRUE, DTpred)
rocdf <- rocdf |> 
  mutate(label = ifelse(threshold %in% seq(0, 1, .01),
                        threshold, NA))
library(ggrepel)
g <- ggplot(rocdf, aes(x = fpr, y = tpr, label = label)) +
  geom_point() +
  geom_path() +
  geom_label_repel(color = "blue", size = 2)
plotly::ggplotly(g)
```